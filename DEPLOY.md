...
| 服务器 OS | - | Ubuntu 22.04+ / **CentOS Stream 9** / Debian 12+ |
| 内存 | 2GB | 后端 512MB + MySQL 512MB + 系统开销 |
| 磁盘 | 20GB | 镜像 + 日志 + 数据增长预留 |

> **CentOS Stream 9 安装 Docker**：
> ```bash
...
## CentOS Stream 9 防火墙配置

CentOS 9 默认使用 `firewalld`，部署前需开放必要端口：

```bash
# 开放 8000 端口（后端 API）
sudo firewall-cmd --permanent --add-port=8000/tcp

# 如果使用 Nginx 反向代理，还需开放 80/443
...
> 注意：生产环境建议将 8000 端口限制为仅本机或内网访问，通过 Nginx 反向代理对外提供服务。
...
| SELinux 阻止容器访问 | CentOS 默认开启 SELinux | `sudo setenforce 0` 临时关闭，或配置 SELinux 策略 |
